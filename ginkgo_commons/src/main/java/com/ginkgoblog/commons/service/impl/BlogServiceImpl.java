...
public class BlogServiceImpl extends ServiceImpl<BlogMapper, Blog> implements BlogService {
    ...
    public List<Blog> setTagAndSortAndPictureByBlogList(List<Blog> list) {
        ...
        picList.forEach(item -> pictureMap.put(item.get("uid").toString(), item.get("url").toString()));
    ...
    public String deleteBatchBlog(List<BlogVO> blogVoList) {
        ...
        blogVoList.forEach(item -> {
            ...
            uidSbf.append(item.getUid()).append(SysConf.FILE_SEGMENTATION);
        });
    ...
    public String uploadLocalBlog(List<MultipartFile> filedatas) throws IOException {
        ...
        if (sysConfig == null) {
            ...
            if (EOpenStatus.OPEN.equals(sysConfig.getUploadQiNiu()) && (StringUtils.isEmpty(sysConfig.getQiNiuPictureBaseUrl()) || StringUtils.isEmpty(sysConfig.getQiNiuAccessKey())
                    || StringUtils.isEmpty(sysConfig.getQiNiuSecretKey()) || StringUtils.isEmpty(sysConfig.getQiNiuBucket()) || StringUtils.isEmpty(sysConfig.getQiNiuArea()))) {
                return ResultUtil.result(SysConf.ERROR, MessageConf.PLEASE_SET_QI_NIU);
            }

            if (EOpenStatus.OPEN.equals(sysConfig.getUploadLocal()) && StringUtils.isEmpty(sysConfig.getLocalPictureBaseUrl())) {
        ...
        for (LinkedTreeMap<String, String> item : list) {
            ...
            if (EOpenStatus.OPEN.equals(sysConfig.getPicturePriority())) {
                // 获取七牛云上的图片
                    ...
                    if (!pictureUrl.startsWith(SysConf.HTTP)) {
                        ...
                        for (Map.Entry<String, String> map : pictureMap.entrySet()) {
                            ...
                            if (pictureUrl.indexOf(map.getKey()) > -1) {
                                if (EOpenStatus.OPEN.equals(sysConfig.getPicturePriority())) {
                                    matchUrlMap.put(pictureUrl, sysConfig.getQiNiuPictureBaseUrl() + map.getValue());
                                } else {
                                    matchUrlMap.put(pictureUrl, sysConfig.getLocalPictureBaseUrl() + map.getValue());
                                }
                                break;
                            }
                        }
